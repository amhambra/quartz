...
SELECT
    ...
    waittime / 1000 AS Wait_Time_Seconds,
    lastwaittype AS Wait_Type,
    Status,
    cmd AS Command_Type,
    loginame AS DB_User,
    hostname AS Client_Machine_Name,
    (SELECT text FROM sys.dm_exec_sql_text((SELECT sql_handle FROM sys.sysprocesses WHERE spid = p.spid))) AS Executing_SQL
FROM sys.sysprocesses p
WHERE spid > 50 AND (blocked <> 0 OR spid IN (SELECT blocked FROM sys.sysprocesses WHERE blocked <> 0));

-- which tables are locked
SELECT 
    OBJECT_NAME(p.object_id) AS Table_Name,
    resource_type AS Lock_Resource,
    request_mode AS Lock_Type, -- Look for X (Exclusive) or IX (Intent Exclusive)
    request_status AS Lock_Status, -- GRANT means holding the lock, WAIT means blocked
    request_session_id AS Session_ID
FROM sys.dm_tran_locks l
JOIN sys.partitions p ON l.resource_associated_entity_id = p.hobt_id
WHERE OBJECT_NAME(p.object_id) LIKE 'QRTZ_%';
